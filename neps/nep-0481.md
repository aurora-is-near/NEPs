...
These host functions are all described in more detail below. Note that the usage of the new `callback` host function in a contract method means that method can only be called synchronously because `callback` panics when used in an VM directly created by a `FunctionCall` action (i.e. async execution).
